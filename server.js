...
const axios = require("axios"); // Used for making HTTP requests to the server
...
async function initializeDataStorage() {
  try {
    ...
    try {
      ...
      console.log(
        "Loaded timezone data for",
        Object.keys(userTimezones).length,
        "users"
      );
    } catch (err) {
      if (err.code !== "ENOENT") {
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // Ensure the server stays awake by pinging it
  try {
  ...
  if (command === "!settimezone") {
    ...
    try {
      ...
      return message.reply(
        "❌ Error setting timezone. Please check your timezone format."
      );
    }
  }
  ...
  if (command === "!mytime") {
    ...
    if (!tz) {
      return message.reply(
        "❌ You haven't set a timezone yet. Use !settimezone first."
      );
    }
